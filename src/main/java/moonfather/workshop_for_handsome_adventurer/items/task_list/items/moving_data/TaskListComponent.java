...
public class TaskListComponent
{
    ...
    public static final Codec<TaskListComponent> CODEC_FOR_COMPONENT = RecordCodecBuilder.create(instance ->
            instance.group(
                    Codec.INT.fieldOf("pageCount").forGetter(TaskListComponent::getPageCount),
                    Codec.INT.fieldOf("lastPage").forGetter(TaskListComponent::getLastPageNumber),
                    Codec.list(TaskListMessaging.CODEC_FOR_PAGE, MAX_PAGE_COUNT, MAX_PAGE_COUNT).fieldOf("pages").forGetter(TaskListComponent::getPages)
            ).apply(instance, TaskListComponent::new)
    );
    public static final StreamCodec<ByteBuf, TaskListComponent> STREAM_CODEC_FOR_COMPONENT = StreamCodec.composite(
            ByteBufCodecs.INT, TaskListComponent::getPageCount,
            ByteBufCodecs.INT, TaskListComponent::getLastPageNumber,
            TaskListMessaging.STREAM_CODEC_FOR_PAGE.apply(ByteBufCodecs.list(MAX_PAGE_COUNT)), TaskListComponent::getPages,
            TaskListComponent::new
    );
    public int getPageCount() { return this.pageCount; }
    public int getLastPageNumber() { return this.lastPage; }
    private List<TaskListMessaging.TaskPageDTO> getPages() { return this.pages; }
    //////////////////////////
