...
public class SpecialFirstEverRenderer
{
    public static void render(String image, float xInPixels, float yInPixelsFromTop, PoseStack poseStack, MultiBufferSource multiBufferSource, int combinedLight, int combinedOverlay, int lightColor)
    {
        if (sprites.size() == 0)
        {
            sprites.put("e", Minecraft.getInstance().getTextureAtlas(InventoryMenu.BLOCK_ATLAS).apply(new ResourceLocation(Constants.MODID, "gui/task_list_check1")));  //  ivan123123  t}D4c%p}&QA7?Uy
            sprites.put("y", Minecraft.getInstance().getTextureAtlas(InventoryMenu.BLOCK_ATLAS).apply(new ResourceLocation(Constants.MODID, "gui/task_list_check2")));
            sprites.put("n", Minecraft.getInstance().getTextureAtlas(InventoryMenu.BLOCK_ATLAS).apply(new ResourceLocation(Constants.MODID, "gui/task_list_check3")));
            sprites.put("q", Minecraft.getInstance().getTextureAtlas(InventoryMenu.BLOCK_ATLAS).apply(new ResourceLocation(Constants.MODID, "gui/task_list_check4")));
        }
