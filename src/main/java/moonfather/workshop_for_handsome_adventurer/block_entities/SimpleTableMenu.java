...
public class SimpleTableMenu extends AbstractContainerMenu
{
			...
			while (! itemStack.isEmpty())
			{
				...
				if (! itemstack.isEmpty() && ItemStack.isSameItemSameTags(itemStack, itemstack))
				{
					...
					if (j <= maxSize)
					{
						...
						itemstack.setCount(j);  // todo ffs rename these two
						slot.set(itemstack);
						result = true;
					}
	...
	public static class UnrestrainedSlot extends Slot  // for storage drawers. will not refuse to add to slots with hundreds of items already there.
	{
		...
		public ItemStack safeInsert(ItemStack stack, int increment)
		{
			...
			int maxSize = this.container.getMaxStackSize();
			int i = Math.min(Math.min(increment, stack.getCount()), maxSize - itemstack.getCount());
